{
	...
	"lastUpdated": "2023-12-11 10:52:22"
}
...
class ID {
	...
	valid() {
		return (this.filename && this.dbname);
	}
}
...
async function scrapePage(doc, url = doc.location.href) {
	...
	newItem.pages = tryMatch(text(doc, '.pages > .item+* span, .pageNum > .item+*'), /[\d-+,]+/);
	newItem.url = url;
	...
	let pdfUrl = attr(doc, 'a.download', 'href');
	if (pdfUrl) {
	...
	addExtra('titleTranslation', text(doc, '.detailTitleEN'));
	switch (newItem.itemType) {
		...
		case 'thesis':
			...
			newItem.thesisType = text(doc, '.degree > .item+*');
			Array.from(doc.querySelectorAll('.tutor > .item+* > a')).forEach((creator) => {
				newItem.creators.push(creator.innerText, 'contributor');
			});
			newItem.date = text(doc, '.thesisYear');
			break;
		...
		case 'patent': {
			...
			addExtra('patentType', text(doc, '.patentType > .item+*'));
		...
		"items": [
			{
				...
				"extra": "titleTranslation: Research progress of microwave assisted biodiesel production",
				"issue": "z1",
		...
		"items": [
			{
				...
				"extra": "titleTranslation: Biological Fermentation Improves the Bioavailability of Asparagus Juice",
				"libraryCatalog": "Wanfang Data",
				"pages": "69-74",
				"place": "广州",
		...
		"items": [
			{
				...
				"extra": "titleTranslation: Quality management systems-Requirementsapply date: 2017-07-01",
				"libraryCatalog": "Wanfang Data",
