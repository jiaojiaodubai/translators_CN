{
	...
	"lastUpdated": "2023-11-30 18:55:30"
}
...
function getSearchResults(doc, checkOnly) {
	...
	let type = MULTITYPE.find(element => element.urlPattern.test(url) && doc.querySelector(element.selector)
	);
	// Z.debug(type);
	if (!type) return false;
	var rows = doc.querySelectorAll(type.selector);
	if (type.selector == '.search-content a') {
		rows = Array.from(rows).filter(element => (element.title || element.querySelector('h3'))
		);
	}
	for (let i = 0; i < rows.length; i++) {
		let row = rows[i];
		let href = [/\/ss\d+/, /\/md\d+/].find(element => element.test(url))
			? `${i}`
			: row.href;
		// Z.debug(href);
...
async function getInfo(url) {
	Z.debug(url);
	let type = SINGLETYPE.find(element => url.includes(element.url));
...
const SINGLETYPE = [
	{
		...
		toItem: function (url, json) {
			...
			tempItem.creators = [matchCreator(
				json.owner.name,
				json.copyright > 1
					? 'contributor'
					: 'director'
			...
			if (json.staff) {
				json.staff.slice(1).forEach(element => tempItem.creators.push(matchCreator(element.name, 'castMember'))
				);
			}
	...
	},
	{
		...
		toItem: function (url, json) {
			...
			tempItem.creators = Array.from(new Set(
				...
				))).map(element => matchCreator(element, 'contributor'));
	...
	},
	{
		...
		toItem: function (url, json) {
			...
			tempItem.creators = [json.up_info.uname].map(element => matchCreator(element, 'director'));
			return tempItem;
		}
	}
];
...
function matchCreator(creator, creatorType) {
	creator = {
		lastName: creator,
